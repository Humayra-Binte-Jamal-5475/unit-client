...
const ApartmentsPage = () => {
  ...
  // Fetch apartments with filters and sorting
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        const res = await axios.get(
          "https://unit-app-server.vercel.app/apartments",
          {
            params,
            signal: ctrl.signal,
          }
    ...
    <main className="max-w-6xl mx-auto px-6 py-10">
      ...
      <div className="flex flex-wrap gap-4 items-end mb-6">
        ...
        <input
          ...
          className="input input-bordered w-32"
        />
        <button className="btn btn-accent" onClick={() => setPage(1)}>Search</button>

        <div className="flex gap-2 ml-auto">
          <button className="btn btn-accent btn-outline" onClick={() => setSortOrder("asc")}>
            Rent ↑
          </button>
          <button className="btn btn-accent btn-outline" onClick={() => setSortOrder("desc")}>
            Rent ↓
          </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {displayedApartments.map((apt) => (
            <div key={apt._id} className="card shadow-lg rounded-lg overflow-hidden">
              <img src={apt.image} alt={apt.apartmentNo} className="h-40 w-full object-cover" />
              <div className="card-body p-4">
                <h3 className="font-bold text-lg mb-1">{apt.block} — Apt {apt.apartmentNo}</h3>
                <p>Floor {apt.floor}</p>
                <p className="font-semibold">৳ {apt.rent}</p>
                <button className="btn btn-accent mt-2 w-full" onClick={() => apply(apt)}>
                  Agreement Request
                </button>
              </div>
            </div>
